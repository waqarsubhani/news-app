<template>
  <v-container grid-list-md="grid-list-md">
    <div class="my-2">
      <v-btn small color="primary" text @click="back">Home</v-btn>
    </div>
    <v-card-text v-if="listings.length">
      <h1>Visited Headlines</h1>
    </v-card-text>
    <v-layout v-if="listings.length" wrap="wrap">
      <v-flex v-for="(listing, index) in listings" xs12="xs12" sm6="sm6" md4="md4" :key="index">
        <v-card v-if="listing.urlToImage">
          <v-img
    ...
    <v-layout v-else wrap="wrap">
      <v-card-text>No Visited Headlines so far</v-card-text>
    </v-layout>
  </v-container>
</template>
...
export default {
  ...
  computed: {
    listings() {
      return this.$store.state.history.articles;
    }
  },
  methods: {
    closeModal() {
